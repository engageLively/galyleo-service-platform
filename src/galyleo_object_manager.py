...
class GalyleoObjectManager:
  ...
  def __init__(self, project, database, namespace, bucket_name):
    ...
    for path in table_paths:
      ...
      self.table_server.add_sdtp_table_from_dictionary(path, table_data["table_dictionary"])
  ...
  def clean_objects(self, kind, user = '*'):
    self.storage_manager.clean_blobs(f'{kind}/{user}/{PATTERN[kind]}')
  ...
  def _validate_table(self, table_dictionary):
    '''
    Validate table dictionary.  When implemented, make sure we can create a table
  ...
  def publish_object(self, galyleo_object,  object_data, share_set = {}):
    ...
    Parameters:
      ...
      object_data: the obejct  file (a JSON string or a dictonary of the appropriate type)
...
ROOT_URL='http://test.galyleo.com'
